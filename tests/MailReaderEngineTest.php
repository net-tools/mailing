...
class MailReaderEngineTest extends \PHPUnit\Framework\TestCase
{
    public function testPlainHtml()
	{
		...
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Multipart', $mail->email);
		$this->assertEquals('alternative', $mail->email->getType());
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\TextPlainContent', $mail->email->getPart(0));
		...
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\TextHtmlContent', $mail->email->getPart(1));
		$this->assertStringStartsWith( 
		...
		$this->assertNotNull($mail->email);
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Multipart', $mail->email);
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Multipart', $mail->email->getPart(0));		
		$this->assertEquals('alternative', $mail->email->getPart(0)->getType());
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\TextPlainContent', $mail->email->getPart(0)->getPart(0));		
		$this->assertStringStartsWith("this is a *unit test* with inline attachment", $mail->email->getPart(0)->getPart(0)->getText());
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Embedding', $mail->email->getPart(1));
		...
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Attachment', $mail->email->getPart(1));
		$fname = $mail->email->getPart(1)->getFile();
		...
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Attachment', $mail->email->getPart(1));
		$this->assertInstanceOf('Nettools\Mailing\MailBuilder\Attachment', $mail->email->getPart(2));
