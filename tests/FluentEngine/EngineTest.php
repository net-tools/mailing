...
/*			->about('Here is the subject line')*/
    ...
    public function testAttachmentsCache()
    {
		...
		$this->assertNotNull($cache->get('idcache'));
    ...
    public function testAttachmentsCacheId()
    {
		$cache = MixedRelated::getCache();
		$cache->clear();
		$c0 = $cache->getCount();
		...
		$this->assertEquals($c0+2, $cache->getCount());	// two attachments are cached (the first one and the third, because cacheId is different : one set, the other computed)
		$this->assertNotNull($cache->get('idcache'));
