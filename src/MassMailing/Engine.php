...
class Engine 
{
	 ...
	 * return \Nettools\Mailing\MassMailing\Engine Returns the calling object for chaining
	 */
	...
	public function ready()
	{
		if ( empty($this->mailer) )
            throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::mailer is not defined");

		if ( empty($this->mail) )
            throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::mail is not defined");
        
		if ( empty($this->mailContentType) )
        	throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::mailContentType is not defined");

		if ( empty($this->from) )
            throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::from is not defined");

		if ( empty($this->template) )
            throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::template is not defined");
		
		
		if ( $this->testMode )
		{
			if ( empty($this->testRecipients) )
            	throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::testRecipients is not defined");
			
			if ( !is_array($this->testRecipients) )
            	throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::testRecipients is not an array");
			
			if ( count($this->testRecipients) == 0 )
            	throw new \Nettools\Mailing\MassMailing\Exception("MassMailing\\Engine::testRecipients is an empty array");
		}
	}
	...
	public function prepareAndSend($mto, $subject = NULL, $data = NULL)
	{
		...
		if ( is_null($dest) )
			throw new \Nettools\Mailing\MassMailing\Exception("Empty email recipient");
		if ( !preg_match("/^[a-z0-9]+([_|\.|-]{1}[a-z0-9]+)*@[a-z0-9]+([_|\.|-]{1}[a-z0-9]+)*[\.]{1}[a-z]{2,6}$/", $dest) )
			throw new \Nettools\Mailing\MassMailing\Exception("Malformed email : '$dest'");
